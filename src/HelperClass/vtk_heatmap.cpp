...
int main(int argc, char* argv[])
{
	...
	string filenames[] = {"cylinder_pathlines_evaluation", "tub_pathlines_evaluation", "blood_flow_evaluation"};
...
void createHeatMap(const std::vector<std::vector<float> >& dataVec)
{
	...
	for(int i=0; i<4; ++i)
	{
		data_range[i][0] = FLT_MAX;
		data_range[i][1] = -FLT_MAX;
		data_range[i][2] = 0;
	}
    ...
    for (int j = rows-1; j >=0; --j)
    {
    	for (int i = 0; i < cols; ++i)
    	{
    		...
    		if(num==0 || num==1)
    			...
    			ofs << (1.0+range_start)-value << std::endl;
...
void create_separate(const std::vector<std::vector<float> >& dataVec)
{
    ...
    string metric[] = {"d_E", "d_F", "d_G", "d_R", "d_M", "d_H", "d_S", "d_P", "d_T"};
    for (int i = 0; i < cols; ++i)
    {
    	if(i%2==0)
    	{
...
void create_ranking(const std::vector<std::vector<float> >& dataVec)
{
	...
	string clustering[]={"K-means", "K-medoids", "AHC-single", "AHC-average", "BIRCH", "DBSCAN", "OPTICS", "SC-kmeans", "SC-eigen", "AP", "PCA"};
...
void create_latex_table(const std::vector<std::vector<float> >& dataVec)
{
	...
	string clustering[] = {"\\textbf{K-means}", "\\textbf{K-medoids}", "\\textbf{AHC}-single", "\\textbf{AHC}-average", "\\textbf{BIRCH}", 
